...
# Game Title
pygame.display.set_caption("Snakes_With_Zeeshan_Haider")
...
def welcome():
    ...
    while not exit_game:
        ...
        text_screen("Welcome to Snakes", black, 260, 190)
        text_screen("Press Space Bar to Play", black, 230, 240)
    ...
    while not exit_game:
        if game_over:
            with open("hiscore.txt", "w") as f:
                f.write(str(hiscore))
            gameWindow.fill((233, 210, 229))
            text_screen("Game Over! Press Enter to Continue", red, 100, 185)
